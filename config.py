...
# 每阶段可独立指定 model / base_url / api_key(甚至跨 provider 混用)。
# 不设则回退到上面的全局三元组 → 单模型用法零变化。
# stage: local=局部摘要(调用量最大),phase=阶段聚合,root=根因判定(最需推理)。
def _stage_env(suffix: str, fallback: str) -> str:
    return os.environ.get(f"LLM_{suffix}") or fallback

LLM_MODEL_LOCAL = _stage_env("MODEL_LOCAL", LLM_MODEL)
LLM_BASE_URL_LOCAL = _stage_env("BASE_URL_LOCAL", LLM_BASE_URL)
LLM_API_KEY_LOCAL = _stage_env("API_KEY_LOCAL", LLM_API_KEY)

LLM_MODEL_PHASE = _stage_env("MODEL_PHASE", LLM_MODEL)
LLM_BASE_URL_PHASE = _stage_env("BASE_URL_PHASE", LLM_BASE_URL)
LLM_API_KEY_PHASE = _stage_env("API_KEY_PHASE", LLM_API_KEY)

LLM_MODEL_ROOT = _stage_env("MODEL_ROOT", LLM_MODEL)
LLM_BASE_URL_ROOT = _stage_env("BASE_URL_ROOT", LLM_BASE_URL)
LLM_API_KEY_ROOT = _stage_env("API_KEY_ROOT", LLM_API_KEY)

# 每阶段三元组(供 llm_client.build_stage_clients 去重建 client)。
STAGE_LLM = {
    "local": {"model": LLM_MODEL_LOCAL, "base_url": LLM_BASE_URL_LOCAL, "api_key": LLM_API_KEY_LOCAL},
    "phase": {"model": LLM_MODEL_PHASE, "base_url": LLM_BASE_URL_PHASE, "api_key": LLM_API_KEY_PHASE},
    "root":  {"model": LLM_MODEL_ROOT,  "base_url": LLM_BASE_URL_ROOT,  "api_key": LLM_API_KEY_ROOT},
}
...
# thinking 的 provider 适配:`thinking`/`reasoning_effort` extra_body 是 **DeepSeek 官方 API 私有参数**,
# 其它 OpenAI 兼容端点(如 SiliconFlow)可能拒绝 → 400。按 base_url 自动判断该用哪种风格:
#   - "deepseek"   : 发 {"thinking":{"type":enabled/disabled}} + reasoning_effort(DeepSeek 官方)
#   - "siliconflow": 发 {"enable_thinking": bool}(SiliconFlow 混合推理模型;非推理模型会忽略/此处仅在显式指定时用)
#   - "none"       : **不注入任何私有 thinking 参数**(最安全,适配任意 OpenAI 兼容端点)
# 留空=按 base_url 推断(deepseek.com→deepseek,其余→none)。可用 env LLM_THINKING_STYLE 强制。
LLM_THINKING_STYLE = (os.environ.get("LLM_THINKING_STYLE") or "").strip().lower() or None
